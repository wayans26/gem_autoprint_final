...
class catalog_location_export implements FromView, WithEvents, WithTitle
{
    ...
    public function view(): View
    {
        ...
        return view('Excel.report_catalog_location', [
            'data'              => $this->data,
    ...
    public function afterSheet(AfterSheet $event)
    {
        ...
        $sheet->getStyle('J8:K' . ($this->lengthRecord + 8))->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('L8:L' . ($this->lengthRecord + 8))->getAlignment()->setWrapText(true)->setVertical(Alignment::VERTICAL_TOP);

        $sheet->getStyle('B8:M' . ($this->lengthRecord + 8))->getAlignment()->applyFromArray([
            'horizontal'    => Alignment::HORIZONTAL_LEFT,
            'vertical'      => Alignment::VERTICAL_TOP
        ]);
        $sheet->getStyle('A8:A' . ($this->lengthRecord + 8))->getAlignment()->applyFromArray([
        ...
        $sheet->getStyle('A8:M' . ($this->lengthRecord + 8))->applyFromArray([
            'borders' => [
                'allBorders' => [
        ...
        if ($this->filter['show_image']) {
            ...
            foreach ($this->data as $key => $value) {
                if ($value->image_path !== null && Storage::exists($value->image_path)) {
                    ...
                    $drawing->setCoordinates('M' . ($key + 9));
                    $drawing->setHeight(120);
                    $drawing->setOffsetY(8);
                    $drawing->setOffsetX(5);
                    $drawing->setWorksheet($sheet);
                    $sheet->getRowDimension($key + 9)->setRowHeight(128);
                }
            }
        }
    }
